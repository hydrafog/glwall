...
bool init_egl(struct glwall_state *state) {
    ...
    if (state->egl_display == EGL_NO_DISPLAY) {
        LOG_ERROR("EGL subsystem error: unable to obtain EGL display");
        return false;
    }

    if (!eglInitialize(state->egl_display, NULL, NULL)) {
        LOG_ERROR("EGL subsystem error: initialization failed");
        return false;
    }

    if (!eglBindAPI(EGL_OPENGL_API)) {
        LOG_ERROR("EGL subsystem error: unable to bind OpenGL API");
        return false;
    }
    ...
    if (!eglChooseConfig(state->egl_display, attribs, &state->egl_config, 1, &num_config) ||
        num_config < 1) {
        LOG_ERROR("EGL subsystem error: unable to select EGL configuration");
        return false;
    }
    LOG_DEBUG(state, "EGL subsystem: configuration selected (index: 0 from %d candidates)", num_config);
    ...
    LOG_DEBUG(state, "EGL subsystem: context created with OpenGL 3.3 Core Profile");

    for (struct glwall_output *output = state->outputs; output; output = output->next) {

        if (!output->wl_egl_window) {
            LOG_ERROR("EGL subsystem error: internal error - EGL window not initialized before context creation");
            return false;
        }
        output->egl_surface =
        ...
        if (output->egl_surface == EGL_NO_SURFACE) {
            LOG_ERROR("EGL subsystem error: unable to create window surface for output %u (EGL error: 0x%x)",
                      output->output_name, eglGetError());
            return false;
        }
    }

    LOG_INFO("EGL subsystem initialization completed successfully");
    return true;
}
