...
static __attribute__((unused)) bool is_pointer_device(struct libevdev *dev) {
...
static bool has_relative_device_for_hardware(struct input_state *input, struct libevdev *dev) {
    ...
    for (int i = 0; i < input->device_count; i++) {
        ...
        if (!input->devices[i].is_absolute && libevdev_get_id_vendor(existing) == vid &&
...
static __attribute__((unused)) bool has_absolute_device_for_hardware(struct input_state *input,
                                                                     struct libevdev *dev) {
    int vid = libevdev_get_id_vendor(dev);
    int pid = libevdev_get_id_product(dev);
...
static __attribute__((unused)) bool try_add_device(struct input_state *input,
...
static bool query_hyprland_cursor(struct input_state *input, double *x, double *y) {
    ...
    size_t path_len = strlen(input->hyprland_socket_path);
    if (path_len >= sizeof(addr.sun_path)) {
        return false;
    }
    memcpy(addr.sun_path, input->hyprland_socket_path, path_len);
...
bool init_input(struct glwall_state *state) {
    LOG_DEBUG(state, "%s",
              "Input subsystem: kernel input device monitoring initialization commenced");

    struct input_state *input = calloc(1, sizeof(struct input_state));
    if (!input) {
        LOG_ERROR("%s", "Memory allocation failed: insufficient memory for input state");
        return false;
    }

    if (state->outputs) {
        input->screen_width = state->outputs->width_px;
        input->screen_height = state->outputs->height_px;
    } else {
    ...
    if (init_hyprland_ipc(input)) {
        LOG_INFO("%s", "Input subsystem: Hyprland IPC initialized for cursor tracking");
    ...
    if (!dir) {
        LOG_WARN("Input subsystem warning: unable to access %s (errno: %s) - kernel input disabled",
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (is_relative_pointer(dev) && input->device_count < MAX_INPUT_DEVICES) {
            ...
            input->device_count++;
            LOG_INFO("Input subsystem: relative pointer device detected (%s: %s)", entry->d_name,
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (is_absolute_pointer(dev) && !is_relative_pointer(dev) &&
            ...
            input->device_count++;
            LOG_INFO("Input subsystem: absolute pointer device detected (%s: %s)", entry->d_name,
                     libevdev_get_name(dev));
        } else {
            libevdev_free(dev);
            close(fd);
    ...
    if (input->device_count == 0) {
        LOG_WARN("Input subsystem warning: no accessible pointer devices found in %s",
                 INPUT_DEVICE_PATH);
        LOG_WARN("%s", "Input subsystem warning: permission denied - kernel input requires read "
                       "access to input devices (add user to 'input' group)");
        free(input);
        return false;
    }

    LOG_INFO("Input subsystem initialization: kernel input enabled with %d pointer device(s)",
             input->device_count);
        ...
        while (1) {
            ...
            if (ev.type == EV_REL) {
                if (ev.code == REL_X) {
                    ...
                    LOG_DEBUG(state,
                              "Input event: REL_X motion detected (delta: %d, position: %.1f)",
                              ev.value, state->pointer_x);
                ...
                if (ev.code == ABS_X) {
                    ...
                    if (abs_info) {
                        ...
                        if (denom <= 0.0) {
                            LOG_WARN("%s",
                                     "Input subsystem: invalid abs range (max == min) for ABS_X");
                        } else {
                    ...
                    if (abs_info) {
                        ...
                        if (denom <= 0.0) {
                            LOG_WARN("%s",
                                     "Input subsystem: invalid abs range (max == min) for ABS_Y");
                        } else {
                            double norm = (double)(ev.value - abs_info->minimum) / denom;
                ...
                if (ev.code == BTN_LEFT) {
                    ...
                    LOG_DEBUG(state,
                              "Input event: BTN_LEFT detected (state: %d, position: %.1f, %.1f)",
                              ev.value, state->pointer_x, state->pointer_y);
                }
            }
        }
    }
...
void cleanup_input(struct glwall_state *state) {
    ...
    LOG_DEBUG(state, "%s", "Input subsystem cleanup initiated");
