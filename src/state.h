...
enum glwall_audio_source {
    ...
    GLWALL_AUDIO_SOURCE_FAKE, // Synthetic audio for debugging/testing
};
...
struct glwall_state {
    ...
    GLuint vao;
    GLint loc_resolution;      // iResolution (vec3)
    GLint loc_resolution_vec2; // resolution (vec2)
    GLint loc_time;
    GLint loc_time_delta;   // iTimeDelta uniform location
    GLint loc_frame;        // iFrame uniform location
    GLint loc_mouse;        // iMouse (vec4)
    GLint loc_mouse_vec2;   // mouse (vec2)
